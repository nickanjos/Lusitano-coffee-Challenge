...
function setup() {
  ...
  // Se precisar anexar a um div específico: cnv.parent('seu-div-id');
  updateGameDimensions();
  resetGame();
  textAlign(CENTER, CENTER);
  textFont('Arial');
  console.log(`Setup: Window=${windowWidth}x${windowHeight}, Canvas=${width}x${height}, Scale=${scaleFactor.toFixed(2)}`);
}
...
function calculateCanvasSize() {
    ...
    if (winW / winH > ASPECT_RATIO) {
        // Window is wider than game aspect ratio => fit height
        canvasHeight = winH;
        canvasWidth = winH * ASPECT_RATIO;
        scaleFactor = winH / BASE_HEIGHT;
    } else {
        // Window is taller or equal aspect ratio => fit width
        canvasWidth = winW;
        canvasHeight = winW / ASPECT_RATIO;
        scaleFactor = winW / BASE_WIDTH;
    }
    // Garantir um tamanho mínimo para evitar erros
    canvasWidth = max(1, floor(canvasWidth));
    canvasHeight = max(1, floor(canvasHeight));
}

// --- Update dimensions dependent on canvas size ---
function updateGameDimensions() {
    groundY = height - (groundHeight * scaleFactor);
    ...
    scaledPlayerJumpForce = playerJumpForce * sqrt(scaleFactor);
    ...
    if (playerImg) {
        ...
        playerGameplayW = max(10, playerGameplayW); // Min width
        playerGameplayH = max(15, playerGameplayH); // Min height
    } else {
        playerGameplayH = max(15, 60 * scaleFactor); // Fallback size
        playerGameplayW = max(10, 40 * scaleFactor);
    }
     // Certifica-te que groundY não é negativo se a tela for muito pequena
     groundY = max(0, groundY);
}


// --- Handles window resize ---
function windowResized() {
  console.log(`Window Resized! New Window=${windowWidth}x${windowHeight}`);
  calculateCanvasSize();
  resizeCanvas(canvasWidth, canvasHeight); // Redimensiona o canvas p5.js
  updateGameDimensions(); // Recalcula tamanhos/posições baseados no novo tamanho
  console.log(`Resized Canvas to: ${width}x${height}, Scale: ${scaleFactor.toFixed(2)}`);

  if (player) {
       player.w = playerGameplayW;
       player.h = playerGameplayH;
       if (player.y + player.h > groundY || !player.isOnGround) { // Se abaixo do chão ou no ar
            player.y = groundY - player.h; // Coloca no chão
            player.velocityY = 0;
            player.isOnGround = true;
       }
  }
  // Forçar um redraw pode ajudar em alguns browsers
   redraw();
}
...
function resetGame() {
  score = 0;
  updateGameDimensions(); // Garante que as dimensões estão atualizadas ANTES de usar scaleFactor
  gameSpeed = initialGameSpeed * scaleFactor;
...
// --- Set Spawn Timers ---
function setNextObstacleSpawn() { nextObstacleSpawnFrame = frameCount + floor(random(minObstacleSpawnInterval, maxObstacleSpawnInterval) / (gameSpeed / (initialGameSpeed * scaleFactor))); } // Ajusta pelo speed atual
function setNextCollectibleSpawn() { nextCollectibleSpawnFrame = frameCount + floor(random(minCollectibleSpawnInterval, maxCollectibleSpawnInterval) / (gameSpeed / (initialGameSpeed * scaleFactor))); } // Ajusta pelo speed atual
...
function draw() {
  ...
  for (let i = 0; i < width; i += (20 * scaleFactor)) { ellipse(i + random(-5, 5) * scaleFactor, groundY + random(5 * scaleFactor, max(6*scaleFactor, (groundHeight * scaleFactor) - (5 * scaleFactor))), random(tuftSize * 0.8, tuftSize * 1.2), random(tuftSize * 0.5, tuftSize)); }

  switch (gameState) {
    case 'start': drawStartScreen(); break;
    case 'playing': if(player) { drawPlayingScreen(); } else { fill(255,0,0); textSize(max(16, 30 * scaleFactor)); text("Error: Player not ready.", width/2, height/2); } break;
    case 'gameOver': drawGameOverScreen(); break;
    case 'error': fill(255,0,0); textSize(max(16, 30 * scaleFactor)); text("Error: Could not load assets.", width/2, height/2); text("Check console (F12) & refresh.", width/2, height/2 + 40 * scaleFactor); break;
  }

  // Draw rotation suggestion if needed (Only if significantly taller than wide)
  if (height > width * 1.2 && (gameState === 'start' || gameState === 'playing' || gameState === 'gameOver')) {
...
function drawRotateSuggestion() {
    ...
    fill(255); textAlign(CENTER, CENTER); textSize(max(18, 20 * scaleFactor));
...
function drawStartScreen() {
  fill(0, 0, 0, 180); rect(width*0.1, height*0.05, width*0.8, height*0.9, max(5, 20 * scaleFactor)); // Ajusta Y e raio
  fill(255);

  // <<< ADICIONAR IMAGEM DO PERSONAGEM >>>
  if (playerImg && playerGameplayW && playerGameplayH) {
      let imgW = playerGameplayW * 0.8; // Um pouco menor que no jogo
      let imgH = playerGameplayH * 0.8;
      image(playerImg, width / 2 - imgW / 2, height * 0.15, imgW, imgH); // Posiciona acima do título
  }

  textFont('Impact'); textSize(max(24, 50 * scaleFactor)); text("THE COFFEE WARRIOR", width / 2, height * 0.40); // Abaixa um pouco o título
  textFont('Arial'); textSize(max(12, 20 * scaleFactor)); text("The journey of an angejense\nin search for the best coffee", width / 2, height * 0.55); // Abaixa
  textSize(max(14, 24 * scaleFactor)); text("TAP SCREEN or Press SPACEBAR to Start", width / 2, height * 0.70); // Abaixa
  textSize(max(10, 16 * scaleFactor)); text("NSCA Games", width / 2, height * 0.85); // Abaixa
  if (bgMusic && bgMusic.isPlaying()) { bgMusic.stop(); }
}
...
function drawPlayingScreen() {
  ...
  player.update();
  gameSpeed = (initialGameSpeed * scaleFactor) + (frameCount * speedIncreaseRate * scaleFactor);
...
function drawGameOverScreen() {
  let enlargedScale = 3; let enlargedW = playerGameplayW * enlargedScale; let enlargedH = playerGameplayH * enlargedScale;
  enlargedW = min(enlargedW, width * 0.5); enlargedH = min(enlargedH, height * 0.5);
  let enlargedX = width / 2 - enlargedW / 2; let enlargedY = height * 0.4 - enlargedH / 2;
  ...
  drawImprovedSpeechBubble("Grandes Merda!", targetX, targetY, max(80, 150 * scaleFactor)); // Passa min width

  let textYPos = enlargedY + enlargedH + 25 * scaleFactor;
  textYPos = max(textYPos, height * 0.65);
...
function drawImprovedSpeechBubble(txt, targetX, targetY, approxWidth) {
    ...
    let scaleFactor = window.scaleFactor || 1; // Acessa a variável global
    ...
    let txtSize = max(10, baseTxtSize * scaleFactor);
    let bubbleStrokeWeight = max(1, baseStrokeWeight * scaleFactor);
    ...
    let bubbleW = max(tw + bubblePadding * 2, approxWidth * 0.6); // Usa approxWidth já escalado
    bubbleW = min(bubbleW, approxWidth * 1.5); // Limita um pouco

    let neededTextHeight = ceil(tw / max(1, (bubbleW - bubblePadding * 2))) * th * 1.3; // Evita divisão por zero
    let bubbleH = neededTextHeight + bubblePadding * 2;
    bubbleH = max(bubbleH, th * 1.3 + bubblePadding * 2); // Garante altura mínima para uma linha

    let bubbleX = targetX + 10 * scaleFactor;
    let bubbleY = targetY - bubbleH * 0.5;

    // Garante que está dentro da tela visível (width e height globais do p5)
    bubbleX = constrain(bubbleX, 10 * scaleFactor, width - bubbleW - 10 * scaleFactor);
    bubbleY = constrain(bubbleY, 10 * scaleFactor, height - bubbleH - 10 * scaleFactor - (groundHeight * scaleFactor));
...
function drawHUD() {
  let hudTextSize = max(12, 20 * scaleFactor);
  let scoreX = 15 * scaleFactor; let scoreY = 15 * scaleFactor;

  let instructionBoxW = max(100, 140 * scaleFactor); // Min width for instruction box
  let instructionBoxH = max(30, 40 * scaleFactor);  // Min height
  let instructionBoxX = width - instructionBoxW - (10 * scaleFactor);
  let instructionBoxY = 10 * scaleFactor;
  let instructionTextSize = max(10, 14 * scaleFactor);

  fill(0); textSize(hudTextSize); textAlign(LEFT, TOP); text("Coffee: " + score, scoreX, scoreY);
  fill(0, 0, 0, 180); rect(instructionBoxX, instructionBoxY, instructionBoxW, instructionBoxH, max(2, 5 * scaleFactor)); // Min radius 2
